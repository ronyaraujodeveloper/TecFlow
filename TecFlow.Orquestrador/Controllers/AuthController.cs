...
public class AuthController : ControllerBase
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            return BadRequest(new { Message = "Provedor de autenticaÃ§Ã£o Ã© obrigatÃ³rio.", ErrorCode = "PROVIDER_REQUIRED" });
        }
        ...
        if (string.Equals(provider, "EmailPassword", StringComparison.OrdinalIgnoreCase))
        {
            ...
            {
                return BadRequest(new { Message = "E-mail e senha sÃ£o obrigatÃ³rios.", ErrorCode = "CREDENTIALS_REQUIRED" });
            }
            ...
            {
                return BadRequest(new { Message = "E-mail invÃ¡lido.", ErrorCode = "INVALID_EMAIL" });
            }

            var usuario = await _userAccountRepository.GetByEmailAsync(email);
            if (usuario is null)
            {
                return Unauthorized(new { Message = "Credenciais invÃ¡lidas.", ErrorCode = "INVALID_CREDENTIALS" });
            }
        ...
        if (string.IsNullOrWhiteSpace(request.AccessToken))
        {
            ...
            {
                Message = $"Token do provedor {provider} Ã© obrigatÃ³rio para login social.",
                ErrorCode = "SOCIAL_TOKEN_REQUIRED"
            });
        }

        _logger.LogInformation(
            "Login social recebido para {Platform} via {Provider}. ValidaÃ§Ã£o do token no provedor serÃ¡ implementada.",
            platform,
            provider);
