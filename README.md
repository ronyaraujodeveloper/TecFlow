...
- [x] **19.2.1. Validação do `PlatformLinkResolver`:** Testar a resolução de domínios nativos da Shopee (`shopee.com.br`) e encurtados (`s.shopee.com.br`, `br.shp.ee`, `shp.ee`, `shope.ee`), com regex desktop `i.{shopId}.{itemId}` (ex. Cadeira Gamer Titans Atlas), sanitização de query ruidosa e HTTP 400 + alerta vermelho no Blazor quando o formato não é reconhecido.
