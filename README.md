...
- [x] **19.2.1. Validação do `PlatformLinkResolver`:** Testar a resolução de domínios nativos da Shopee (`shopee.com.br`) e encurtados (`s.shopee.com.br`, `br.shp.ee`, `shp.ee`, `shope.ee`), com extração flexível de `ItemId`/`ShopId` (`*-i.{shopId}.{itemId}`), sanitização de query ruidosa e fallback de homologação (`999999`/`888888`) quando a URL expandida não carrega os IDs.
